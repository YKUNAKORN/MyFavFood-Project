document.querySelectorAll('.checkout button').forEach(button => {
    button.addEventListener('click', () => {
        ...
        else if (button.classList.contains('total-btn')) {
            window.location.href = '#'; 
        } 
                        ...
                        <tbody>
                            ${displayedItems.map(item => `
                                <tr>
                                    ...
                                    <td class="tb-right">${Number(item.sand_price).toFixed(2)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
...
async function saveOrder() {
  ...
  try {
      const response = await fetch('http://localhost:3000/api/menu/save-order', {
          method: 'POST',
          headers: {
...
{/* <div class="menu-item">
${item.sand_name}       ${Number(item.sand_price).toFixed(2)}
</div> */}