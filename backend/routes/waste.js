...
module.exports = (pool) => {
    ...
    router.get('/:menuId', async (req, res) => {
        ...
        try {
            const [rows] = await pool.query(
                'SELECT sand_name FROM sandwich WHERE sand_name = ?',
                [menuId]
            );
    ...
    router.post('/waste-order', async (req, res) => {
        ...
        try {
            ...
            const values = orderItems.map(item => [item.sand_name]);
            const query = 'INSERT INTO wastebasket (item_id ) VALUES ?';
